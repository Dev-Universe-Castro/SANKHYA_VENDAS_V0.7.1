...
export function ProdutoSelectorModal({
  ...
  const buscarProdutos = async (termo: string) => {
    ...
    try {
      ...
      if (cachedProdutos) {
        try {
          ...
          const filtered = allProdutos.filter((p: any) =>
            p.DESCRPROD.toLowerCase().includes(termo.toLowerCase()) ||
            p.CODPROD.includes(termo)
          ).slice(0, 20)
      ...
      const url = `/api/sankhya/produtos/search?q=${encodeURIComponent(termo)}&limit=20`;
      console.log('📡 URL:', url);
      ...
      console.log('✅ Produtos definidos:', data.produtos?.length || 0);
    } catch (error) {
