...
async function analisarDadosDoSistema(userId: number, userName: string) {
  try {
    ...
    // Buscar dados em paralelo com timeout de 15 segundos cada
    const [leadsData, parceirosData, produtosData, pedidosData] = await Promise.allSettled([
      ...
      fetchWithTimeout(`${baseUrl}/api/sankhya/parceiros?page=1&pageSize=20`, {}, 15000)
        .then(r => r.ok ? r.json() : { parceiros: [] }).catch(() => ({ parceiros: [] })),
      
      fetchWithTimeout(`${baseUrl}/api/sankhya/produtos?page=1&pageSize=30`, {}, 15000)
        .then(r => r.ok ? r.json() : { produtos: [] }).catch(() => ({ produtos: [] })),
    ...
    if (parceirosData.status === 'rejected') console.error('❌ Falha ao carregar parceiros:', parceirosData.reason);
    if (produtosData.status === 'rejected') console.error('❌ Falha ao carregar produtos:', produtosData.reason);
